...
<% from zk.model import EventType %>
<% count = 4 %>
...
%     if int(i.id) == int(vot.event_id):
  <div class="votecard">
  <h4>VOTE CAST</h4>
  <p>You have cast one of your votes for:<br/> <br/>${i.computed_title()} ${("(%s)" % i.computed_speakers()[0]) if len(i.computed_speakers()) else ""}</p>
...
<% presentation_type = EventType.find_by_name('presentation') %>

% for t in c.time_slot:
%   for s in c.schedule:
%     if s.time_slot_id == t.id:
%       for i in c.events:
%         if i.id == s.event_id and i.type_id == presentation_type.id and i.computed_title() not in ['Keynote 1','Keynote 2','Sysadmin Miniconf','Best Of #1','Best Of #2','Best Of #3','Best Of #4']:
   <div class="votecard ${t.start_time.strftime("%a")}"><h4>${t.start_time.strftime("%a %I:%M%p")}</h4>
	   <p> ${i.computed_title()} ${("(%s)" % i.computed_speakers()[0]) if len(i.computed_speakers()) else ""} </p>
%           if count > 0:
